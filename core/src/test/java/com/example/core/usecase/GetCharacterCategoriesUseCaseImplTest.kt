...
class GetCharacterCategoriesUseCaseImplTest {
    ...
    fun `should return Success from ResultStatus when get both requests return success`() =
        runTest {
            // Arrange
            whenever(repository.getComics(character.characterId)).thenReturn(comics)
            whenever(repository.getEvents(character.characterId)).thenReturn(events)

            // Act
            val result = getCharacterCategoriesUseCase
                .invoke(GetCharacterCategoriesUseCase.GetCategoriesParams(character.characterId))
    ...
    fun `should return Error from ResultStatus when get events request returns error`() =
        runTest {
            // Arrange
            whenever(repository.getComics(character.characterId)).thenAnswer { throw Throwable() }
    ...
    fun `should return Error from ResultStatus when get comics request returns error`() =
        runTest {
            // Arrange
            whenever(repository.getComics(character.characterId)).thenReturn(comics)
            whenever(repository.getEvents(character.characterId)).thenAnswer { throw Throwable() }
